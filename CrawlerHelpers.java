...
public interface CrawlerHelpers {
    ...
    */
    public static void addDataToFile(String url, Data parsedData){
        ...
        file.println(parsedData.getLinks());
    ...
    public static List<String> parseHtml(String string,
        ...
        while (activeLoop) {
            ...
            if (pStartIndex != -1){
                ...
                words.append(' ').append(string.substring(pStartIndex + 3, endIndex).replace("\\n", " "));
                string = string.replace(string.substring(pStartIndex + 3,endIndex),"");
            }
            //for <a > tag
        ...
        String fulllinks = getLinks(links.toString(), url, linkQueue);
        for (String word : words.toString().strip().split(" ")){
        ...
        return Arrays.asList(title, words.toString(), fulllinks);
    }
